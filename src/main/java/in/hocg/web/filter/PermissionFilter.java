...
public class PermissionFilter implements Serializable {
     ...
     * 更新 与 增加 均拥有
     */
    @NotBlank(message = "名称为必填", groups = {
            Update.class, Insert.class
    })
    private String name; // 名称: 用户添加
    @Range(min = 0, max = 1, message = "资源类型错误", groups = {
            Update.class, Insert.class
    })
    private Integer type; // 资源类型 0 菜单 1 按钮
    private String url;  // 资源路径.
    @NotBlank(message = "权限标识为必填", groups = {
            Update.class, Insert.class
    })
    @Pattern(regexp="^[a-zA-Z\\._]+", message = "权限标识只允许由英文、点、下划线组成", groups = {
            Update.class, Insert.class
    })
