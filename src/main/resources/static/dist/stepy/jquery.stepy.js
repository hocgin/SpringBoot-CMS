    ...
    var methods = {
        init : function(settings) {
            return this.each(function() {
                ...
                var self = this, that = $(this), id = that.attr("id");
                if (id === undefined || id === "") {
                ...
                if (self.opt.titleClick) {
                    self.heads.click(function() {
                        var array = self.heads.filter(".stepy-active").attr(
                            "id").split("-"), current = parseInt(
                            array[array.length - 1], 10), clicked = $(this)
                            .index();
                        if (clicked > current) {
                            if (self.opt.next
                                && !methods._execute.call(that,
                                    self.opt.next, clicked)) {
                                return false
                            }
                        } else {
                            if (clicked < current) {
                ...
                self.steps.first().find(":input:visible:enabled").first()
                    .select().focus();
                that.data({
                    settings : this.opt,
                    stepy : true
        ...
        _bindEnter : function() {
            ...
            self.steps.delegate('input[type="text"], input[type="password"]',
                "keypress", function(evt) {
                    ...
                    if (key == 13) {
                        evt.preventDefault();
                        var buttons = $(this).closest("fieldset").find(
                            ".stepy-navigator");
                        if (buttons.length) {
        ...
        _bindFinish : function() {
            var self = this, that = $(this), finish = that
                .children('input[type="submit"]');
            self.finish = (finish.length === 1) ? finish : that
                .children(".stepy-finish");
            if (self.finish.length) {
                var isForm = that.is("form"), onSubmit = undefined;
                        ...
                        function(evt) {
                            if (self.opt.finish
                                ...
                                if (isForm) {
                                    ...
                                    var isSubmit = self.finish
                                        .attr("type") === "submit";
                                    if (!isSubmit
                                        && (!self.opt.validate || methods.validate
                                            .call(
                                                that,
                                                self.steps.length - 1))) {
                                        that.submit()
                ...
                self.steps.last().children(".stepy-navigator").append(
                    self.finish)
            } else {
                $
        ...
        _createBackButton : function(nav, index) {
            var self = this, that = $(this), attributes = {
                href : "#",
                "class" : "button-back btn btn-primary",
                html : self.opt.backLabel
            };
            $("<a />", attributes).on(
                "click.stepy",
                function(e) {
                    e.preventDefault();
                    if (!self.opt.back
                        || methods._execute.call(self, self.opt.back,
                            index - 1)) {
        ...
        _createHead : function(step, index) {
            var step = $(step)
                .attr("id", $(this).attr("id") + "-step-" + index)
                .addClass("stepy-step"), head = methods._head.call(this,
                index);
        ...
        _createNextButton : function(nav, index) {
            var self = this, that = $(this), attributes = {
                href : "#",
                "class" : "button-next btn btn-primary",
                html : self.opt.nextLabel
            };
            $("<a/>", attributes).on(
                "click.stepy",
                function(e) {
                    e.preventDefault();
                    if (!self.opt.next
                        || methods._execute.call(that, self.opt.next,
                            index + 1)) {
                        methods.step.call(self, (index + 1) + 1)
                    }
                }).appendTo(nav)
        ...
        _head : function(index) {
            return $("<li />", {
                id : $(this).attr("id") + "-head-" + index
            })
        },
        _header : function() {
            var header = $("<ul />", {
                id : $(this).attr("id") + "-header",
                "class" : "stepy-header"
            });
        ...
        destroy : function() {
            return $(this).each(
                function() {
                    var that = $(this);
                    if (that.data("stepy")) {
                        var steps = that.data("stepy", false).children(
        ...
        step : function(index) {
            ...
            that[0].heads.removeClass("stepy-active").eq(max).addClass(
                "stepy-active");
            if (that.is("form")) {
        ...
        validate : function(index) {
            ...
            var self = this, step = that.children("fieldset").eq(index), isValid = true, $title = $(
                "#" + that.attr("id") + "-header").children().eq(index), $validate = that
                .validate();
            $(step.find(":input[validate='true']").get().reverse()).each(function() {
